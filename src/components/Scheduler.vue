        ...
        <div class="grid" :style="setHeight">
          ...
          <template v-for='i in getShiftTimeArray[selectedPersonIndex]'>
            ...
            <template v-for="n in 7">
              <SchedulerItem v-if="n === selectedDay && i === selectedHour" class="selected" :date='currentWeek[n - 1]'
                :hour='i' :tasks='filteredTasks[n - 1]' :person='people[selectedPersonIndex]'
                :breakTime='getBreakTimeArray[selectedPersonIndex]' :timeScale='timeScale' @timeSelected="timeSelection"
                @taskDeletion="deleteTask" />
              <SchedulerItem v-else :date='currentWeek[n - 1]' :hour='i' :tasks='filteredTasks[n - 1]'
                :person='people[selectedPersonIndex]' :breakTime='getBreakTimeArray[selectedPersonIndex]'
                :timeScale='timeScale' @timeSelected="timeSelection" @taskDeletion="deleteTask" />
            </template>
          </template>
        </div>
      </div>
      <Tasks :resolution='getResolutionHeight' :date='selectedDateFormatting' :tasks='filteredTasks[selectedDay - 1]'
        :hour='selectedHour' :formattedTime='formatTime' :breakTime='getBreakTimeArray[selectedPersonIndex]'
        :shiftTime='getShiftTimeArray[selectedPersonIndex]' :timeScale='timeScale' @NewTask="createNewTask"
        @taskDeletion="deleteTask" />
    </div>
  </div>
</template>
